<script setup lang="ts">
import type { BlogPost } from '~/types'

const { data: page } = await useAsyncData('blog', () => queryContent('/posts').findOne())
if (!page.value) {
  throw createError({ statusCode: 404, statusMessage: 'Page not found', fatal: true })
}

const { data: posts } = await useAsyncData('posts', () => queryContent<BlogPost>('/posts')
  .where({ _extension: 'md' })
  .sort({ date: -1 })
  .find())

useSeoMeta({
  title: page.value.title,
  ogTitle: page.value.title,
  description: page.value.description,
  ogDescription: page.value.description
})

defineOgImage({
  component: 'Saas',
  title: page.value.title,
  description: page.value.description
})
const activePost = useState()
</script>

<template>
  <UContainer>
    <UPageHero v-bind="page">
      <NuxtImg v-bind="page?.image" sizes="400px" class="hidden lg:block mx-auto rounded-md shadow-xl ring-1 ring-gray-300 dark:ring-gray-700"/>
    </UPageHero>

    <UPageBody>
      <UBlogList>
        <UBlogPost @click="activePost = index"
          v-for="(post, index) in posts"
          :key="index"
          :to="post._path"
          :title="post.title"
          :description="post.description"
          :image="post.image"
          :date="new Date(post.date).toLocaleDateString('en', { year: 'numeric', month: 'short', day: 'numeric' })"
          :authors="post.authors"
          :badge="post.badge"
          :orientation="index === 0 ? 'horizontal' : 'vertical'"
          :class="[index === 0 && 'col-span-full', activePost === index && 'active']"
          :ui="{
            description: 'line-clamp-2'
          }"
        >
          <!-- <template #date>
              <time :datetime="post.date" class="ext-sm text-gray-500 dark:text-gray-400 font-medium pointer-events-none">
                {{  new Date(post.date).toLocaleDateString('en', { year: 'numeric', month: 'short', day: 'numeric' }) }}
              </time>
          </template> -->
        </UBlogPost>
      </UBlogList>
    </UPageBody>
  </UContainer>
</template>

<style scoped>
  .active {
    view-transition-name: selected-post;
    contain: layout;
  }
</style>
